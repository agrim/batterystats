...
final class MenuBarBatteryLabelFormattingTests: XCTestCase {
    ...
    func testLabelStateRecomputesFromCurrentDisplayPreferences() {
        let suiteName = "MenuBarBatteryLabelFormattingTests-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        let preferences = PreferencesStore(defaults: defaults, sync: NoopPreferencesSync())
        preferences.menuBarDisplayMode = .iconOnly
    ...
    func testAppInstallsExplicitStatusItemForDisplayPreferenceChanges() throws {
        ...
        XCTAssertTrue(appSource.contains("configureWindowForCurrentSpace(window)"))
        XCTAssertTrue(appSource.contains("behavior.remove(.canJoinAllSpaces)"))
        XCTAssertTrue(appSource.contains(".canJoinAllApplications"))
        XCTAssertTrue(appSource.contains(".fullScreenAuxiliary"))
        XCTAssertTrue(appSource.contains(".moveToActiveSpace"))
        ...
        XCTAssertTrue(menuBarSource.contains("private static let panelCollectionBehavior: NSWindow.CollectionBehavior"))
        ...
        XCTAssertTrue(menuBarSource.contains(".canJoinAllApplications"))
        XCTAssertTrue(menuBarSource.contains(".fullScreenAuxiliary"))
        XCTAssertTrue(menuBarSource.contains(".moveToActiveSpace"))
        ...
        XCTAssertTrue(menuBarSource.contains(".transient"))
        ...
        XCTAssertTrue(menuBarSource.contains("panel.collectionBehavior = Self.panelCollectionBehavior"))
    ...
    func testMainDashboardWindowCanMoveToCurrentFullScreenSpace() throws {
        ...
        XCTAssertTrue(dashboardSource.contains(".background(CurrentWindowSpaceConfigurator())"))
        XCTAssertTrue(dashboardSource.contains("NSViewRepresentable"))
        XCTAssertTrue(dashboardSource.contains("windowConfigurationRetryDelays"))
        XCTAssertTrue(dashboardSource.contains("Task { @MainActor"))
        XCTAssertTrue(dashboardSource.contains("Task.sleep"))
        XCTAssertTrue(dashboardSource.contains("configureWindowIfAvailable(for: view)"))
        XCTAssertTrue(dashboardSource.contains("behavior.remove(.canJoinAllSpaces)"))
        XCTAssertTrue(dashboardSource.contains(".canJoinAllApplications"))
        XCTAssertTrue(dashboardSource.contains(".fullScreenAuxiliary"))
    ...
    func testMenuBarPanelPresentationUsesFullScreenAuxiliaryNonActivatingWindow() throws {
        let suiteName = "MenuBarPanelPresentationTests-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        let preferences = PreferencesStore(defaults: defaults, sync: NoopPreferencesSync())
        let monitor = BatteryMonitor()
    ...
    func testMenuBarPanelOutsideClickDuringOpeningGraceStillDismissesAfterGrace() async throws {
        let suiteName = "MenuBarPanelDismissalGraceTests-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        let preferences = PreferencesStore(defaults: defaults, sync: NoopPreferencesSync())
        let monitor = BatteryMonitor()
    ...
    func testMenuBarPanelInsideInteractionCancelsDeferredDismissal() async throws {
        let suiteName = "MenuBarPanelDismissalCancellationTests-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        let preferences = PreferencesStore(defaults: defaults, sync: NoopPreferencesSync())
        let monitor = BatteryMonitor()
    ...
    func testDisplayPreferenceInvalidationFromUnrelatedStoreDoesNotChangeStatusItem() async {
        let runtimeSuiteName = "MenuBarStatusItemRuntimeInvalidationTests-\(UUID().uuidString)"
        let unrelatedSuiteName = "MenuBarStatusItemUnrelatedInvalidationTests-\(UUID().uuidString)"
        let runtimeDefaults = UserDefaults(suiteName: runtimeSuiteName)!
        let unrelatedDefaults = UserDefaults(suiteName: unrelatedSuiteName)!
        runtimeDefaults.removePersistentDomain(forName: runtimeSuiteName)
